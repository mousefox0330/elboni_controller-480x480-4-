...
void ui_sr_anim_start(lv_obj_t *parent)
{
    ...
    lv_label_set_text_static(g_sr_label, (sys_set->lang ? "倾听中..." : "Listening..."));
    ...
    lv_label_set_text_static(lab_hint, (sys_set->lang ? "恢复出厂设置" : "Touch to enter factory reset"));
...
static void sr_layer_timer_cb(lv_timer_t *tmr)
{
    ...
    if (pdPASS == app_lvgl_get_event(&lvgl_event, 0)) {
        ...
        case LV_EVENT_I_AM_LEAVING:
            sr_anim_stop();
            set_tips_info(sys_set->lang ? "倾听中..." : "Listening...");
            break;
