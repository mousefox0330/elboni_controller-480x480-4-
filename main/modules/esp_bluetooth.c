...
#define TAG "GATTC"
#if GATTC_DEBUG
	#define ELBONI_DBG_PRINT_GATTC(...) ESP_LOGI(__VA_ARGS__)
#else
	#define ELBONI_DBG_PRINT_GATTC(...)
#endif
...
{
  while (1) {
    ELBONI_DBG_PRINT_GATTC(TAG, "led_task is running");
	if (task_run) {
		if (toggle == 0) {
			uint8_t val[] = {'o', 'n'};
...
static void gattc_profile_event_handler(esp_gattc_cb_event_t event, esp_gatt_if_t gattc_if, esp_ble_gattc_cb_param_t *param)
{
    ...
    case ESP_GATTC_REG_EVT:
        ELBONI_DBG_PRINT_GATTC(TAG, "GATT client register, status %d, app_id %d, gattc_if %d", param->reg.status, param->reg.app_id, gattc_if);
    ...
    case ESP_GATTC_CONNECT_EVT:{
        ELBONI_DBG_PRINT_GATTC(TAG, "Connected, conn_id %d, remote "ESP_BD_ADDR_STR"", p_data->connect.conn_id,
    ...
    case ESP_GATTC_DIS_SRVC_CMPL_EVT:
        ...
        ELBONI_DBG_PRINT_GATTC(TAG, "Service discover complete, conn_id %d", param->dis_srvc_cmpl.conn_id);
        esp_ble_gattc_search_service(gattc_if, param->dis_srvc_cmpl.conn_id, &remote_filter_service_uuid);
        break;
    case ESP_GATTC_CFG_MTU_EVT:
        ELBONI_DBG_PRINT_GATTC(TAG, "MTU exchange, status %d, MTU %d", param->cfg_mtu.status, param->cfg_mtu.mtu);
        break;
    case ESP_GATTC_SEARCH_RES_EVT: {
        ELBONI_DBG_PRINT_GATTC(TAG, "Service search result, conn_id = %x, is primary service %d", p_data->search_res.conn_id, p_data->search_res.is_primary);
        ELBONI_DBG_PRINT_GATTC(TAG, "start handle %d, end handle %d, current handle value %d", p_data->search_res.start_handle, p_data->search_res.end_handle, p_data->search_res.srvc_id.inst_id);
        if (p_data->search_res.srvc_id.uuid.len == ESP_UUID_LEN_16 && p_data->search_res.srvc_id.uuid.uuid.uuid16 == REMOTE_SERVICE_UUID) {
            ELBONI_DBG_PRINT_GATTC(TAG, "Service found");
            get_server = true;
            gl_profile_tab[PROFILE_A_APP_ID].service_start_handle = p_data->search_res.start_handle;
            gl_profile_tab[PROFILE_A_APP_ID].service_end_handle = p_data->search_res.end_handle;
            ELBONI_DBG_PRINT_GATTC(TAG, "UUID16: %x", p_data->search_res.srvc_id.uuid.uuid.uuid16);
        }
        break;
    }
    case ESP_GATTC_SEARCH_CMPL_EVT:
        ...
        if(p_data->search_cmpl.searched_service_source == ESP_GATT_SERVICE_FROM_REMOTE_DEVICE) {
            ELBONI_DBG_PRINT_GATTC(TAG, "Get service information from remote device");
        } else if (p_data->search_cmpl.searched_service_source == ESP_GATT_SERVICE_FROM_NVS_FLASH) {
            ELBONI_DBG_PRINT_GATTC(TAG, "Get service information from flash");
        } else {
            ELBONI_DBG_PRINT_GATTC(TAG, "Unknown service source");
        }
        ELBONI_DBG_PRINT_GATTC(TAG, "Service search complete");
        if (get_server){
            uint16_t count = 0;
    ...
    case ESP_GATTC_REG_FOR_NOTIFY_EVT: {
        if (p_data->reg_for_notify.status != ESP_GATT_OK){
            ...
            ELBONI_DBG_PRINT_GATTC(TAG, "Notification register successfully");
            uint16_t count = 0;
    ...
    case ESP_GATTC_NOTIFY_EVT:
        if (p_data->notify.is_notify){
            ELBONI_DBG_PRINT_GATTC(TAG, "Notification received");
        }else{
            ELBONI_DBG_PRINT_GATTC(TAG, "Indication received");
        }
    ...
    case ESP_GATTC_WRITE_DESCR_EVT:
        ...
        ELBONI_DBG_PRINT_GATTC(TAG, "Descriptor write successfully");
    ...
    case ESP_GATTC_SRVC_CHG_EVT: {
        ...
        ELBONI_DBG_PRINT_GATTC(TAG, "Service change from "ESP_BD_ADDR_STR"", ESP_BD_ADDR_HEX(bda));
        break;
    }
    case ESP_GATTC_WRITE_CHAR_EVT:
        ...
        ELBONI_DBG_PRINT_GATTC(TAG, "Characteristic write successfully");
        break;
...
static void esp_gap_cb(esp_gap_ble_cb_event_t event, esp_ble_gap_cb_param_t *param)
{
    ...
    case ESP_GAP_BLE_SCAN_RESULT_EVT: {
        ...
        case ESP_GAP_SEARCH_INQ_RES_EVT:
            ...
            if (scan_result->scan_rst.scan_rsp_len > 0) {
                ELBONI_DBG_PRINT_GATTC(TAG, "scan resp:");
            ...
            if (adv_name != NULL) {
                if (strlen(remote_device_name) == adv_name_len && strncmp((char *)adv_name, remote_device_name, adv_name_len) == 0) {
                    ...
                    ELBONI_DBG_PRINT_GATTC(TAG, "Device found %s", remote_device_name);
                    if (connect == false) {
                        connect = true;
    ...
    case ESP_GAP_BLE_SCAN_STOP_COMPLETE_EVT:
        ...
        ELBONI_DBG_PRINT_GATTC(TAG, "Scanning stop successfully");
        break;
    ...
    case ESP_GAP_BLE_ADV_STOP_COMPLETE_EVT:
        ...
        ELBONI_DBG_PRINT_GATTC(TAG, "Advertising stop successfully");
        break;
    case ESP_GAP_BLE_UPDATE_CONN_PARAMS_EVT:
         ELBONI_DBG_PRINT_GATTC(TAG, "Connection params update, status %d, conn_int %d, latency %d, timeout %d",
    ...
    case ESP_GAP_BLE_SET_PKT_LENGTH_COMPLETE_EVT:
        ELBONI_DBG_PRINT_GATTC(TAG, "Packet length update, status %d, rx %d, tx %d",
...
static void esp_gattc_cb(esp_gattc_cb_event_t event, esp_gatt_if_t gattc_if, esp_ble_gattc_cb_param_t *param)
{
    ...
    if (event == ESP_GATTC_REG_EVT) {
        if (param->reg.status == ESP_GATT_OK) {
            ...
            ELBONI_DBG_PRINT_GATTC(TAG, "reg app failed, app_id %04x, status %d",
                    param->reg.app_id,
                    param->reg.status);
            return;
        }
